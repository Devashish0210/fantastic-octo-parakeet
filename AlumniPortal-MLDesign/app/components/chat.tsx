...
export const Chat = () => {
  ...
  const [ticketCacheTimestamp, setTicketCacheTimestamp] = useState<number | null>(null);
  const [isTicketsPrefetching, setIsTicketsPrefetching] = useState(false);

  // ============== NDC Pre-fetch State ==============
const [ndcCache, setNdcCache] = useState<any>(null);
const [ndcCacheTimestamp, setNdcCacheTimestamp] = useState<number | null>(null);
...
const [hasNdcFetched, setHasNdcFetched] = useState(false);
  ...
  useEffect(() => {
    ...
    console.log("🔍 [REDUX DEBUG] Current employeeDetails:", {
      empID: employeeDetails?.empID,
      name: employeeDetails?.name,
      doj: employeeDetails?.doj,
      lwd: employeeDetails?.lwd,
      timestamp: new Date().toISOString()
    });
    ...
    console.log("👤 [NAME DEBUG] Extracted name:", {
      fullName,
      firstName,
      timestamp: new Date().toISOString()
    });
...
const prefetchNDC = useCallback(async () => {
  if (!employeeLoginState?.empID || !employeeLoginState?.email || !employeeLoginState?.otp) {
    return;
  }
  ...
  try {
    ...
    const normalizeStatus = (status: string) => {
      return (status === "Unknown" || status === "unknown") ? "Pending" : status;
    };
    ...
    // console.log(`✅ NDC pre-fetched successfully - ${transformedData.departments.length} departments`);
    
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
      if (error.response) {
        console.warn("⚠️ NDC pre-fetch failed with status:", error.response.status);
...
useEffect(() => {
  ...
  const {
    ...
    onFinish: (message) => {
      console.log("✅ [CHAT] Message finished:", {
        ...
        // hasToolCalls: message.toolInvocations?.length > 0,
        timestamp: new Date().toISOString()
      });
    },
    ...
    onError: async (error) => {
      ...
      let errorMessage = "I encountered an unexpected error. Please try again or contact HR for assistance.";
      ...
      try {
        ...
        if (errorMessage.includes("session has expired") || errorMessage.includes("Redirecting to login")) {
          shouldRedirect = true;
          errorType = "AUTH_ERROR";
        } else if (errorMessage.includes("can't help with that request")) {
          errorType = "CONTENT_FILTER";
        } else if (errorMessage.includes("connectivity issues") || errorMessage.includes("connection")) {
  ...
  const messages = rawMessages.filter((message) => message.content.trim() !== "");
  ...
  if (!isAuthenticating && (!employeeLoginState?.empID || !employeeLoginState?.email || !employeeLoginState?.otp)) {
    return (
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      {/* ⭐ Pass sessionId to Header for Feedback Modal */}
      <Header onClearChat={clearChat} sessionId={sessionId} />

      {/* Optional: Show cache indicator in dev mode */}
      {process.env.NODE_ENV === "development" && ticketCache && ticketCache.length > 0 && (
  ...
  <div style={{
    ...
    ✅ NDC cached • 
    {ndcCache.departments?.length || 0} departments • 
    Age: {ndcCacheTimestamp ? Math.round((Date.now() - ndcCacheTimestamp) / 1000) : 0}s
  </div>
)}
